...
$total_pages_sql = "SELECT COUNT(*) FROM `applications`";
$result = mysqli_query($link,$total_pages_sql);
$total_rows = mysqli_fetch_array($result)[0];
$total_pages = ceil($total_rows / $no_of_records_per_page);

//Column sorting on column name
$columns = array('id', 'user_id', 'job_id', 'company_id', 'status', 'score');
...
if (!empty($_GET['search'])) {
    ...
    if (strpos('`applications`.`id`, `applications`.`user_id`, `applications`.`job_id`, `applications`.`company_id`, `applications`.`status`, `applications`.`score`', ',')) {
        $where_statement .= " AND CONCAT_WS (`applications`.`id`, `applications`.`user_id`, `applications`.`job_id`, `applications`.`company_id`, `applications`.`status`, `applications`.`score`) LIKE '%$search%'";
    } else {
        $where_statement .= " AND `applications`.`id`, `applications`.`user_id`, `applications`.`job_id`, `applications`.`company_id`, `applications`.`status`, `applications`.`score` LIKE '%$search%'";
    }
...
$group_clause = !empty($order) && $order == 'id' ? "GROUP BY `applications`.`$order`" : '';
...
if ($isCsvExport) {
    ...
    exportAsCSV($data, $db_name, $tables_and_columns_names, 'applications', $link, false);
    exit;
}
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        ...
                        <a href="applications-create.php" class="btn btn-success float-right"><?php translate('Add New Record') ?></a>
                        <a href="applications-index.php" class="btn btn-info float-right mr-2"><?php translate('Reset View') ?></a>
                        <a href="applications-index.php?export=csv" class="btn btn-primary float-right mr-2"><?php translate('Export as CSV') ?></a>
                        <a href="javascript:history.back()" class="btn btn-secondary float-right mr-2"><?php translate('Back') ?></a>
                    </div>

                    <div class="form-row">
                        <form action="applications-index.php" method="get">
                        ...
                        if($result) :
                            if(mysqli_num_rows($result) > 0) :
                                ...
                                <table class='table table-bordered table-striped'>
									...
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									echo "<th><a href=?search=$search&order=user_id&sort=".$sort_link.">user_id</a></th>";
									$columnname = "job_id";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									echo "<th><a href=?search=$search&order=job_id&sort=".$sort_link.">job_id</a></th>";
									$columnname = "company_id";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									echo "<th><a href=?search=$search&order=company_id&sort=".$sort_link.">company_id</a></th>";
									$columnname = "status";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									...
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									echo "<th><a href=?search=$search&order=score&sort=".$sort_link.">score</a></th>";
 ?>
										...
										echo "<td>" . htmlspecialchars($row['company_id'] ?? "") . "</td>";
										 ...
										 ?>
                                                <td>
                                                    ...
                                                    $column_id = 'id';
                                                    if (!empty($column_id)): ?>
                                                        <a id='read-<?php echo $row['id']; ?>' href='applications-read.php?id=<?php echo $row['id']; ?>' title='<?php echo addslashes(translate('View Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-info'><i class='far fa-eye'></i></a>
                                                        <a id='update-<?php echo $row['id']; ?>' href='applications-update.php?id=<?php echo $row['id']; ?>' title='<?php echo addslashes(translate('Update Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-warning'><i class='far fa-edit'></i></a>
                                                        <a id='delete-<?php echo $row['id']; ?>' href='applications-delete.php?id=<?php echo $row['id']; ?>' title='<?php echo addslashes(translate('Delete Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-danger'><i class='far fa-trash-alt'></i></a>
                                                    <?php else: ?>
