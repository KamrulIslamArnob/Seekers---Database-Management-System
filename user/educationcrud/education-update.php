...
if(isset($_POST["edu_id"]) && !empty($_POST["edu_id"])){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
		...
		$degree = $_POST["degree"] == "" ? null : trim($_POST["degree"]);
        ...
        $stmt = $link->prepare("UPDATE `education` SET `user_id`=?,`name`=?,`degree`=?,`city`=?,`start_date`=?,`end_date`=?,`description`=? WHERE `edu_id`=?");

        try {
            $stmt->execute([ $user_id, $name, $degree, $city, $start_date, $end_date, $description, $edu_id  ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            header("location: education-read.php?edu_id=$edu_id");
        } else {
                ...
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                                            ...
                                            <input type="text" name="user_id" id="user_id" maxlength="255" class="form-control" value="<?php echo @$user_id; ?>" readonly>
						...
						<div class="form-group">
                                            ...
                                            <input type="text" name="degree" id="degree" maxlength="255" class="form-control" value="<?php echo @$degree; ?>">
                                        </div>
						<div class="form-group">
                                            <label for="city">City</label>
                                            <input type="text" name="city" id="city" maxlength="255" class="form-control" value="<?php echo @$city; ?>">
                                        </div>
						<div class="form-group">
                        ...
                        <hr>
                        <p>
                            <a href="education-read.php?edu_id=<?php echo $_GET["edu_id"];?>" class="btn btn-info"><?php translate('View Record') ?></a>
