...
if(isset($_POST["project_id"]) && !empty($_POST["project_id"])){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
		...
		$id = trim($_POST["id"]);
		$project_keyword = $_POST["project_keyword"] == "" ? null : trim($_POST["project_keyword"]);
		$Languages = $_POST["Languages"] == "" ? null : trim($_POST["Languages"]);
		$Client = $_POST["Client"] == "" ? null : trim($_POST["Client"]);
		

        // Prepare an update statement

        $stmt = $link->prepare("UPDATE `projects` SET `project_name`=?,`description`=?,`project_link`=?,`id`=?,`project_keyword`=?,`Languages`=?,`Client`=? WHERE `project_id`=?");

        try {
            $stmt->execute([ $project_name, $description, $project_link, $id, $project_keyword, $Languages, $Client, $project_id  ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            header("location: projects-read.php?project_id=$project_id");
        } else {
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) == 1){
                ...
                $project_name = htmlspecialchars($row["project_name"] ?? "");
					...
					$project_link = htmlspecialchars($row["project_link"] ?? "");
					$id = htmlspecialchars($row["id"] ?? "");
					$project_keyword = htmlspecialchars($row["project_keyword"] ?? "");
                ...
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                                            ...
                                            <input type="text" name="project_name" id="project_name" maxlength="255" class="form-control" value="<?php echo @$project_name; ?>">
                                        </div>
						<div class="form-group">
						...
						<div class="form-group">
                                            ...
                                            <input type="text" name="project_link" id="project_link" maxlength="100" class="form-control" value="<?php echo @$project_link; ?>">
                                        </div>
						<div class="form-group">
                                            <label for="id">id*</label>
                                            <input type="text" name="id" id="id" maxlength="255" class="form-control" value="<?php echo @$user_id; ?>" readonly>
                                           
						<div class="form-group">
                                            <label for="project_keyword">Genre</label>
                                            <input type="text" name="project_keyword" id="project_keyword" maxlength="255" class="form-control" value="<?php echo @$project_keyword; ?>">
                                        </div>
						<div class="form-group">
                                            <label for="Languages">Languages / Tools</label>
                                            <input type="text" name="Languages" id="Languages" maxlength="255" class="form-control" value="<?php echo @$Languages; ?>">
                                        </div>
						<div class="form-group">
                                            <label for="Client">Client</label>
                                            <input type="text" name="Client" id="Client" maxlength="255" class="form-control" value="<?php echo @$Client; ?>">
                                        </div>
                        ...
                        <hr>
                        <p>
                            <a href="projects-read.php?project_id=<?php echo $_GET["project_id"];?>" class="btn btn-info"><?php translate('View Record') ?></a>
                            <a href="projects-delete.php?project_id=<?php echo $_GET["project_id"];?>" class="btn btn-danger"><?php translate('Delete Record') ?></a>
                            <a href="projects-index.php" class="btn btn-primary"><?php translate('Back to List') ?></a>
                        </p>
