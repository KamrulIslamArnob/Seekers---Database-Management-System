...
$columns = array('blog_title', 'blog_tagline', 'blog_text', 'blog_genre', 'blog_cover', 'created_at');
...
if (!empty($_GET['search'])) {
    ...
    if (strpos('`blogs`.`blog_title`, `blogs`.`blog_tagline`, `blogs`.`blog_text`, `blogs`.`blog_genre`, `blogs`.`blog_cover`, `blogs`.`created_at`', ',')) {
        $where_statement .= " AND CONCAT_WS (`blogs`.`blog_title`, `blogs`.`blog_tagline`, `blogs`.`blog_text`, `blogs`.`blog_genre`, `blogs`.`blog_cover`, `blogs`.`created_at`) LIKE '%$search%'";
    } else {
        $where_statement .= " AND `blogs`.`blog_title`, `blogs`.`blog_tagline`, `blogs`.`blog_text`, `blogs`.`blog_genre`, `blogs`.`blog_cover`, `blogs`.`created_at` LIKE '%$search%'";
    }
...
$group_clause = !empty($order) && $order == 'blog_id' ? "GROUP BY `blogs`.`$order`" : '';
...
if ($isCsvExport) {
    ...
    exportAsCSV($data, $db_name, $tables_and_columns_names, 'blogs', $link, false);
    exit;
}
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        ...
                        <a href="blogs-create.php" class="btn btn-success float-right"><?php translate('Add New Record') ?></a>
                        <a href="blogs-index.php" class="btn btn-info float-right mr-2"><?php translate('Reset View') ?></a>
                        <a href="blogs-index.php?export=csv" class="btn btn-primary float-right mr-2"><?php translate('Export as CSV') ?></a>
                        <a href="javascript:history.back()" class="btn btn-secondary float-right mr-2"><?php translate('Back') ?></a>
                    </div>

                    <div class="form-row">
                        <form action="blogs-index.php" method="get">
                        ...
                        if($result) :
                            if(mysqli_num_rows($result) > 0) :
                                ...
                                <table class='table table-bordered table-striped'>
									...
									echo "<th><a href=?search=$search&order=blog_title&sort=".$sort_link.">Ttile</a></th>";
									$columnname = "blog_tagline";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									echo "<th><a href=?search=$search&order=blog_tagline&sort=".$sort_link.">About</a></th>";
									$columnname = "blog_text";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									echo "<th><a href=?search=$search&order=blog_text&sort=".$sort_link.">Section</a></th>";
									$columnname = "blog_genre";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									echo "<th><a href=?search=$search&order=blog_genre&sort=".$sort_link.">Genre</a></th>";
									$columnname = "blog_cover";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									echo "<th><a href=?search=$search&order=blog_cover&sort=".$sort_link.">Cover Img</a></th>";
									$columnname = "created_at";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									echo "<th><a href=?search=$search&order=created_at&sort=".$sort_link.">Created</a></th>";
 ?>
                                    ...
                                    <tbody>
                                        <?php while($row = mysqli_fetch_array($result)): ?>
											...
											echo nl2br(htmlspecialchars($row['blog_title'] ?? ""));
											if ($has_link_file){
											    echo $is_file ? "</a>" : "";
											}
											echo "</td>"."\n\t\t\t\t\t\t\t\t\t\t\t\t";											echo "<td>";
											...
											echo nl2br(htmlspecialchars($row['blog_tagline'] ?? ""));
											if ($has_link_file){
											    echo $is_file ? "</a>" : "";
											}
											echo "</td>"."\n\t\t\t\t\t\t\t\t\t\t\t\t";											echo "<td>";
											...
											echo nl2br(htmlspecialchars($row['blog_text'] ?? ""));
											if ($has_link_file){
											    echo $is_file ? "</a>" : "";
											}
											echo "</td>"."\n\t\t\t\t\t\t\t\t\t\t\t\t";											echo "<td>";
											...
											echo nl2br(htmlspecialchars($row['blog_genre'] ?? ""));
											if ($has_link_file){
											    echo $is_file ? "</a>" : "";
											}
											echo "</td>"."\n\t\t\t\t\t\t\t\t\t\t\t\t";											echo "<td>";
											...
											echo nl2br(htmlspecialchars($row['blog_cover'] ?? ""));
											if ($has_link_file){
											    echo $is_file ? "</a>" : "";
											}
										 ...
										 ?>
                                                <td>
                                                    ...
                                                    $column_id = 'blog_id';
                                                    if (!empty($column_id)): ?>
                                                        <a id='read-<?php echo $row['blog_id']; ?>' href='blogs-read.php?blog_id=<?php echo $row['blog_id']; ?>' title='<?php echo addslashes(translate('View Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-info'><i class='far fa-eye'></i></a>
                                                        <a id='update-<?php echo $row['blog_id']; ?>' href='blogs-update.php?blog_id=<?php echo $row['blog_id']; ?>' title='<?php echo addslashes(translate('Update Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-warning'><i class='far fa-edit'></i></a>
                                                        <a id='delete-<?php echo $row['blog_id']; ?>' href='blogs-delete.php?blog_id=<?php echo $row['blog_id']; ?>' title='<?php echo addslashes(translate('Delete Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-danger'><i class='far fa-trash-alt'></i></a>
                                                    <?php else: ?>
