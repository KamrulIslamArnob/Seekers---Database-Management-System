...
if(isset($_POST["skill_id"]) && !empty($_POST["skill_id"])){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
		...
		$skill_name = $_POST["skill_name"] == "" ? null : trim($_POST["skill_name"]);
		$skill_level = $_POST["skill_level"] == "" ? null : trim($_POST["skill_level"]);
		

        // Prepare an update statement

        $stmt = $link->prepare("UPDATE `skills` SET `user_id`=?,`skill_name`=?,`skill_level`=? WHERE `skill_id`=?");

        try {
            $stmt->execute([ $user_id, $skill_name, $skill_level, $skill_id  ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            header("location: skills-read.php?skill_id=$skill_id");
        } else {
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) == 1){
                ...
                $user_id = htmlspecialchars($row["user_id"] ?? "");
					...
					$skill_level = htmlspecialchars($row["skill_level"] ?? "");
                ...
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
						...
						<div class="form-group">
                                            ...
                                            <input type="text" name="skill_name" id="skill_name" maxlength="255" class="form-control" value="<?php echo @$skill_name; ?>">
                                        </div>
						<div class="form-group">
                                            <label for="skill_level">Proficiency</label>
                                            <input type="text" name="skill_level" id="skill_level" maxlength="50" class="form-control" value="<?php echo @$skill_level; ?>">
                                        </div>
                        ...
                        <hr>
                        <p>
                            <a href="skills-read.php?skill_id=<?php echo $_GET["skill_id"];?>" class="btn btn-info"><?php translate('View Record') ?></a>
