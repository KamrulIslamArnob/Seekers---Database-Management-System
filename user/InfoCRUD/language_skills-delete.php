...
if(isset($_POST["language_id"]) && !empty($_POST["language_id"])){
    ...
    $fileColumns = [];
    if (isset($tables_and_columns_names['language_skills']['columns'])) {
        foreach ($tables_and_columns_names['language_skills']['columns'] as $columnName => $columnDetails) {
    ...
    $sql = "DELETE FROM `language_skills` WHERE `language_id` = ?";
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if (!isset($error)){
            ...
            header("location: language_skills-index.php");
        }
    }
