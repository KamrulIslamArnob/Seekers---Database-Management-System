...
if(isset($_POST["service_id"]) && !empty($_POST["service_id"])){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
		...
		$service_name = $_POST["service_name"] == "" ? null : trim($_POST["service_name"]);
		$service_description = $_POST["service_description"] == "" ? null : trim($_POST["service_description"]);
		$service_taken = $_POST["service_taken"] == "" ? null : trim($_POST["service_taken"]);
		

        // Prepare an update statement

        $stmt = $link->prepare("UPDATE `services` SET `service_id`=?,`user_id`=?,`service_name`=?,`service_description`=?,`service_taken`=? WHERE `service_id`=?");

        try {
            $stmt->execute([ $service_id, $user_id, $service_name, $service_description, $service_taken, $service_id  ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            header("location: services-read.php?service_id=$service_id");
        } else {
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) == 1){
                ...
                $service_id = htmlspecialchars($row["service_id"] ?? "");
					...
					$service_taken = htmlspecialchars($row["service_taken"] ?? "");
                ...
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
						...
						<div class="form-group">
                                            ...
                                            <input type="number" name="user_id" id="user_id" class="form-control" value="<?php echo @$user_id; ?>" readonly>
                                        </div>
						<div class="form-group">
                                            <label for="service_name">Title</label>
                                            <input type="text" name="service_name" id="service_name" maxlength="255" class="form-control" value="<?php echo @$service_name; ?>">
                                        </div>
						<div class="form-group">
                                            <label for="service_description">Description</label>
                                            <textarea name="service_description" id="service_description" class="form-control"><?php echo @$service_description; ?></textarea>
                                        </div>
                        ...
                        <hr>
                        <p>
                            <a href="services-read.php?service_id=<?php echo $_GET["service_id"];?>" class="btn btn-info"><?php translate('View Record') ?></a>
