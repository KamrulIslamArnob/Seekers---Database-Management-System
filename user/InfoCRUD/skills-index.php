...
$total_pages_sql = "SELECT COUNT(*) FROM `skills`";
$result = mysqli_query($link, $total_pages_sql);
$total_rows = mysqli_fetch_array($result)[0];
$total_pages = ceil($total_rows / $no_of_records_per_page);

//Column sorting on column name
$columns = array('skill_id', 'skill_name', 'skill_level');
...
if (!empty($_GET['search'])) {
    ...
    if (strpos('`skills`.`skill_id`, `skills`.`skill_name`, `skills`.`skill_level`', ',')) {
        $where_statement .= " AND CONCAT_WS (`skills`.`skill_id`, `skills`.`skill_name`, `skills`.`skill_level`) LIKE '%$search%'";
    } else {
        $where_statement .= " AND `skills`.`skill_id`, `skills`.`skill_name`, `skills`.`skill_level` LIKE '%$search%'";
    }
...
$group_clause = !empty($order) && $order == 'skill_id' ? "GROUP BY `skills`.`$order`" : '';

// Prepare SQL queries
$sql = "SELECT skills.*
FROM skills
LEFT JOIN users AS user_idusers ON user_idusers.id = skills.user_id
WHERE user_idusers.id = $user_id 
...
if ($isCsvExport) {
    ...
    exportAsCSV($data, $db_name, $tables_and_columns_names, 'skills', $link, false);
    exit;
}
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        ...
                        <a href="skills-create.php" class="btn btn-success float-right">
                            <?php translate('Add New Record') ?>
                        </a>
                        <a href="skills-index.php" class="btn btn-info float-right mr-2">
                            <?php translate('Reset View') ?>
                        </a>
                        <a href="skills-index.php?export=csv" class="btn btn-primary float-right mr-2">
                                ...
                                <table class='table table-bordered table-striped'>
                                    <thead class='thead-light'>
                                        <tr>
                                            ...
                                            echo "<th><a href=?search=$search&order=skill_name&sort=" . $sort_link . ">Name</a></th>";
                                            $columnname = "skill_level";
                                            $sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
                                            $sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
                                            echo "<th><a href=?search=$search&order=skill_level&sort=" . $sort_link . ">Proficiency</a></th>";
                                            ?>
                                            <th>
                                    ...
                                    <tbody>
                                        <?php while ($row = mysqli_fetch_array($result)): ?>
                                            <tr>
                                                ...
                                                echo nl2br(htmlspecialchars($row['skill_name'] ?? ""));
                                                if ($has_link_file) {
                                                    echo $is_file ? "</a>" : "";
                                                }
                                                echo "</td>" . "\n\t\t\t\t\t\t\t\t\t\t\t\t";
                                                ...
                                                echo nl2br(htmlspecialchars($row['skill_level'] ?? ""));
                                                if ($has_link_file) {
                                                    echo $is_file ? "</a>" : "";
                                                }
                                                echo "</td>" . "\n\t\t\t\t\t\t\t\t\t\t\t\t"; ?>
                                                <td>
                                                    ...
                                                    $column_id = 'skill_id';
                                                    if (!empty($column_id)): ?>
                                                        <a id='read-<?php echo $row['skill_id']; ?>'
                                                            href='skills-read.php?skill_id=<?php echo $row['skill_id']; ?>'
                                                            title='<?php echo addslashes(translate('View Record', false)); ?>'
                                                            data-toggle='tooltip' class='btn btn-sm btn-info'><i
                                                                class='far fa-eye'></i></a>
                                                        <a id='update-<?php echo $row['skill_id']; ?>'
                                                            href='skills-update.php?skill_id=<?php echo $row['skill_id']; ?>'
                                                            title='<?php echo addslashes(translate('Update Record', false)); ?>'
                                                            data-toggle='tooltip' class='btn btn-sm btn-warning'><i
                                                                class='far fa-edit'></i></a>
                                                        <a id='delete-<?php echo $row['skill_id']; ?>'
                                                            href='skills-delete.php?skill_id=<?php echo $row['skill_id']; ?>'
