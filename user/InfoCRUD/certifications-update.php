...
if (isset($_POST["certification_id"]) && !empty($_POST["certification_id"])) {
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
        ...
        $stmt = $link->prepare("UPDATE `certifications` SET `certification_id`=?,`certification_name`=?,`issuing_authority`=?,`issue_date`=?,`expiration_date`=?,`id`=? WHERE `certification_id`=?");

        try {
            $stmt->execute([$certification_id, $certification_name, $issuing_authority, $issue_date, $expiration_date, $id, $certification_id]);
        } catch (Exception $e) {
        ...
        if (!isset($error)) {
            header("location: certifications-read.php?certification_id=$certification_id");
        } else {
...
if (isset($_GET["certification_id"]) && !empty($_GET["certification_id"])) {
    // Get URL parameter
    $certification_id = trim($_GET["certification_id"]);

    // Prepare a select statement
    $sql = "SELECT * FROM `certifications` WHERE `certification_id` = ?";
    if ($stmt = mysqli_prepare($link, $sql)) {
        // Set parameters
        $param_id = $certification_id;
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            if (mysqli_num_rows($result) == 1) {
                ...
                $certification_id = htmlspecialchars($row["certification_id"] ?? "");
                $certification_name = htmlspecialchars($row["certification_name"] ?? "");
                $issuing_authority = htmlspecialchars($row["issuing_authority"] ?? "");
                ...
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post"
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="certification_name" id="certification_name" maxlength="255"
                                class="form-control" value="<?php echo @$certification_name; ?>">
                        </div>
                        <div class="form-group">
                            ...
                            <input type="hidden" name="cruddiy_backup_issuing_authority"
                                id="cruddiy_backup_issuing_authority" value="<?php echo @$issuing_authority; ?>">
                            <?php if (isset($issuing_authority) && !empty($issuing_authority)): ?>
                                <div class="custom-control custom-checkbox">
                                    <input type="checkbox" class="custom-control-input"
                                        id="cruddiy_delete_issuing_authority" name="cruddiy_delete_issuing_authority"
                                        value="1">
                                    <label class="custom-control-label" for="cruddiy_delete_issuing_authority">
                                        <?php translate("Delete:") ?>: <a href="uploads/<?php echo $issuing_authority ?>"
                                            target="_blank">
                                            <?php echo $issuing_authority ?>
                                        </a>
                                    </label>
                                </div>
                            <?php endif ?>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="id" id="id" maxlength="255" class="form-control"
                        ...
                        <hr>
                        <p>
                            <a href="certifications-read.php?certification_id=<?php echo $_GET["certification_id"]; ?>"
                                class="btn btn-info">
