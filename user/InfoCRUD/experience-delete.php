...
if(isset($_POST["experience_id"]) && !empty($_POST["experience_id"])){


    // Find uploaded files references for deletion
    $fileColumns = [];
    if (isset($tables_and_columns_names['experience']['columns'])) {
        foreach ($tables_and_columns_names['experience']['columns'] as $columnName => $columnDetails) {
    ...
    if (!empty($fileColumns)) {
        foreach ($fileColumns as $columnName) {
            ...
            if ($stmt = mysqli_prepare($link, $sql)) {
                // Set parameters
                $param_id = trim($_POST["experience_id"]);
    ...
    $sql = "DELETE FROM `experience` WHERE `experience_id` = ?";
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if (!isset($error)){
            ...
            header("location: experience-index.php");
        }
    }
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . "?experience_id=" . $_GET["experience_id"]; ?>" method="post">
                    <?php print_error_if_exists(@$error); ?>
                        <div class="alert alert-danger fade-in">
                            <input type="hidden" name="experience_id" value="<?php echo trim($_GET["experience_id"]); ?>"/>
                    ...
                    <hr>
                    <p>
                        <a href="experience-read.php?experience_id=<?php echo $_GET["experience_id"];?>" class="btn btn-info"><?php translate('View Record') ?></a>
