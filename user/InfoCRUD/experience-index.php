...
$total_pages_sql = "SELECT COUNT(*) FROM `experience`";
$result = mysqli_query($link, $total_pages_sql);
$total_rows = mysqli_fetch_array($result)[0];
$total_pages = ceil($total_rows / $no_of_records_per_page);

//Column sorting on column name
$columns = array('experience_id', 'job_title', 'company', 'start_date', 'end_date', 'description');
...
if (!empty($_GET['search'])) {
    ...
    if (strpos('`experience`.`experience_id`, `experience`.`job_title`, `experience`.`company`, `experience`.`start_date`, `experience`.`end_date`, `experience`.`description`', ',')) {
        $where_statement .= " AND CONCAT_WS (`experience`.`experience_id`, `experience`.`job_title`, `experience`.`company`, `experience`.`start_date`, `experience`.`end_date`, `experience`.`description`) LIKE '%$search%'";
    } else {
        $where_statement .= " AND `experience`.`experience_id`, `experience`.`job_title`, `experience`.`company`, `experience`.`start_date`, `experience`.`end_date`, `experience`.`description` LIKE '%$search%'";
    }
...
$group_clause = !empty($order) && $order == 'experience_id' ? "GROUP BY `experience`.`$order`" : '';

// Prepare SQL queries
$sql = "SELECT experience.*
FROM experience
LEFT JOIN users ON users.id = experience.id
...
if ($isCsvExport) {
    ...
    exportAsCSV($data, $db_name, $tables_and_columns_names, 'experience', $link, false);
    exit;
}
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        ...
                        <a href="experience-create.php" class="btn btn-success float-right">
                            <?php translate('Add New Record') ?>
                        </a>
                        <a href="experience-index.php" class="btn btn-info float-right mr-2">
                            <?php translate('Reset View') ?>
                        </a>
                        <a href="experience-index.php?export=csv" class="btn btn-primary float-right mr-2">
                    ...
                    <div class="form-row">
                        <form action="experience-index.php" method="get">
                                ...
                                <table class='table table-bordered table-striped'>
                                    <thead class='thead-light'>
                                        <tr>
                                            ...
                                            // echo "<th><a href=?search=$search&order=experience_id&sort=" . $sort_link . ">Experience  Number</a></th>";
                                            $columnname = "job_title";
                                            $sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
                                            $sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
                                            echo "<th><a href=?search=$search&order=job_title&sort=" . $sort_link . ">Title</a></th>";
                                            $columnname = "company";
                                            $sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
                                            $sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
                                            echo "<th><a href=?search=$search&order=company&sort=" . $sort_link . ">Company/Organizations</a></th>";
                                    ...
                                    <tbody>
                                        <?php while ($row = mysqli_fetch_array($result)): ?>
                                            <tr>
                                                ...
                                                echo nl2br(htmlspecialchars($row['job_title'] ?? ""));
                                                if ($has_link_file) {
                                                    echo $is_file ? "</a>" : "";
                                                }
                                                echo "</td>" . "\n\t\t\t\t\t\t\t\t\t\t\t\t";
                                                ...
                                                echo nl2br(htmlspecialchars($row['company'] ?? ""));
                                                if ($has_link_file) {
                                                    echo $is_file ? "</a>" : "";
                                                }
                                                echo "</td>" . "\n\t\t\t\t\t\t\t\t\t\t\t\t";
                                                ...
                                                <td>
                                                    <?php
                                                    $column_id = 'experience_id';
                                                    if (!empty($column_id)): ?>
                                                        <a id='read-<?php echo $row['experience_id']; ?>'
                                                            href='experience-read.php?experience_id=<?php echo $row['experience_id']; ?>'
                                                            title='<?php echo addslashes(translate('View Record', false)); ?>'
                                                            data-toggle='tooltip' class='btn btn-sm btn-info'><i
                                                                class='far fa-eye'></i></a>
                                                        <a id='update-<?php echo $row['experience_id']; ?>'
                                                            href='experience-update.php?experience_id=<?php echo $row['experience_id']; ?>'
                                                            title='<?php echo addslashes(translate('Update Record', false)); ?>'
                                                            data-toggle='tooltip' class='btn btn-sm btn-warning'><i
                                                                class='far fa-edit'></i></a>
                                                        <a id='delete-<?php echo $row['experience_id']; ?>'
                                                            href='experience-delete.php?experience_id=<?php echo $row['experience_id']; ?>'
