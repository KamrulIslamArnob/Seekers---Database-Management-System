...
if(isset($_POST["certification_id"]) && !empty($_POST["certification_id"])){
    ...
    $fileColumns = [];
    if (isset($tables_and_columns_names['certifications']['columns'])) {
        foreach ($tables_and_columns_names['certifications']['columns'] as $columnName => $columnDetails) {
    ...
    if (!empty($fileColumns)) {
        foreach ($fileColumns as $columnName) {
            ...
            if ($stmt = mysqli_prepare($link, $sql)) {
                // Set parameters
                $param_id = trim($_POST["certification_id"]);
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if (!isset($error)){
            ...
            header("location: certifications-index.php");
        }
    }
    ...
    if(empty($_GET["certification_id"])){
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . "?certification_id=" . $_GET["certification_id"]; ?>" method="post">
                    <?php print_error_if_exists(@$error); ?>
                        <div class="alert alert-danger fade-in">
                            <input type="hidden" name="certification_id" value="<?php echo trim($_GET["certification_id"]); ?>"/>
                    ...
                    <hr>
                    <p>
                        <a href="certifications-read.php?certification_id=<?php echo $_GET["certification_id"];?>" class="btn btn-info"><?php translate('View Record') ?></a>
                        <a href="certifications-update.php?certification_id=<?php echo $_GET["certification_id"];?>" class="btn btn-warning"><?php translate('Update Record') ?></a>
